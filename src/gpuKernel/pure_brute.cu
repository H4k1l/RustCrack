...
extern "C" __global__ void crack_word(char* chars, int charsLen, unsigned long long* range, char* hash, unsigned char* hashtype, int tothashes, int n, bool verbose, bool nofile, char* output_file) {
            ...
            for (int k = 0; k < tothashes; k++) {
                switch (hashtype[k]) {
                    case 0:
                        ...
                        if (last_hashlen != hashlen) // recompute the hash only if the length has changed
                            compute_md5_cuda_single(word, current_length, hash_word_cmp);
                        break;
                    case 1:
                        // sha-1
                        hashlen = 40;
                        ...
                        if (last_hashlen != hashlen)
                            compute_sha1_cuda_single(word, current_length, hash_word_cmp);
                        break;
                    case 2:
                        //  sha-224
                        hashlen = 56;
                        // conversion
                        if (last_hashlen != hashlen)
                            compute_sha224_cuda_single(word, current_length, hash_word_cmp);
                        break;
                    case 3:
                        //  sha-256
                        hashlen = 64;
                        // conversion
                        if (last_hashlen != hashlen)
                            compute_sha256_cuda_single(word, current_length, hash_word_cmp);
                        break;
                    case 4:
                        // sha-384
                        hashlen = 96;
                        // conversion
                        if (last_hashlen != hashlen)
                            compute_sha384_cuda_single(word, current_length, hash_word_cmp);
                        break;
                    case 5:
                        // sha-512
                        hashlen = 128;
                        // conversion
                        if (last_hashlen != hashlen)
                            compute_sha512_cuda_single(word, current_length, hash_word_cmp);
                        break;
                }
                ...
                if (match[k]) { 
                    ...
                    if (!nofile) { // if not disabled, insert the entries in the buffer, in the format: <\0hash\0word\0>
                        ...
                        int rec_size = hashlen + current_length + 4;
                        ...
                        for (int l = 0; l < current_length && word[l] != '\0'; l++){
                            output_file[offset+1+hashlen+1+l] = word[l];
                        }
                        output_file[(offset+1+hashlen+1+current_length)] = '\0';
                    }
                }
